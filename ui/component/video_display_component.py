...
class VideoDisplayComponent(QWidget):
    ...
    def video_coordinates_to_preview_coordinates(self, video_selection_rects):
        """Convert original video coordinates to preview ratio coordinates."""
        selection_rects = []
        if not self.frame_width or not self.frame_height:
            return selection_rects

        scaled_width = self.scaled_width if self.scaled_width else 1
        scaled_height = self.scaled_height if self.scaled_height else 1
        for rect in video_selection_rects:
            ymin, ymax, xmin, xmax = rect
            preview_xmin = self.border_left + (xmin / self.frame_width) * scaled_width
            preview_xmax = self.border_left + (xmax / self.frame_width) * scaled_width
            preview_ymin = self.border_top + (ymin / self.frame_height) * scaled_height
            preview_ymax = self.border_top + (ymax / self.frame_height) * scaled_height

            preview_xmin = max(0, min(preview_xmin, 1))
            preview_xmax = max(0, min(preview_xmax, 1))
            preview_ymin = max(0, min(preview_ymin, 1))
            preview_ymax = max(0, min(preview_ymax, 1))
