...
def _cluster_by_y_center(boxes, tolerance):
    ...
    for box in sorted(boxes, key=lambda item: (item[2] + item[3]) / 2):
        ...
        for cluster in clusters:
            cluster_center = sum((item[2] + item[3]) / 2 for item in cluster) / len(cluster)
...
def auto_detect_subtitle_area(video_path, sample_count=36, bottom_start_ratio=0.52):
    ...
    def cluster_score(cluster):
        widths = [box[1] - box[0] for box in cluster]
        centers = [(box[2] + box[3]) / 2 for box in cluster]
        return len(cluster) * (1.0 + np.median(widths) / max(1, frame_width)) * (1.0 + np.median(centers) / max(1, frame_height) * 0.2)

    primary = max(clusters, key=cluster_score)
    primary_center = np.median([(box[2] + box[3]) / 2 for box in primary])
    primary_score = cluster_score(primary)
    selected_boxes = list(primary)
    for cluster in clusters:
        if cluster is primary:
            continue
        cluster_center = np.median([(box[2] + box[3]) / 2 for box in cluster])
        if abs(cluster_center - primary_center) <= frame_height * 0.14 and cluster_score(cluster) >= primary_score * 0.35:
            selected_boxes.extend(cluster)

    xmins = [box[0] for box in selected_boxes]
    xmaxs = [box[1] for box in selected_boxes]
